...
(function initHomeCategories(){
  ...
  var homeStyle=document.createElement('style');homeStyle.textContent='.tools-wrap{overflow-y:auto}.tool-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.tool-card{position:relative;overflow:hidden;min-height:155px;transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease}.tool-card:hover{transform:translateY(-4px);border-color:#db0011;box-shadow:0 14px 28px rgba(31,48,62,.14)}.tool-card:hover .tool-icon{transform:scale(1.08);background:#ffe8ea}.tool-icon{transition:transform .2s ease,background .2s ease}.tool-workflow{position:absolute;top:12px;right:14px;color:#9aa5af;font-size:9px;font-weight:700;letter-spacing:1px;text-transform:uppercase}.tool-card:hover .tool-workflow{color:#db0011}@media(max-width:980px){.tool-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:620px){.tool-grid{grid-template-columns:1fr}}';document.head.appendChild(homeStyle);
