...
(function initHomeCategories(){
  ...
  var homeStyle=document.createElement('style');homeStyle.textContent='.tools-wrap{overflow-y:auto}.tool-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.tool-card{position:relative;overflow:hidden;min-height:135px;padding:15px;transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease}.tool-card:hover{transform:translateY(-3px);border-color:#db0011;box-shadow:0 11px 22px rgba(31,48,62,.13)}.tool-card:hover .tool-icon{transform:scale(1.06);background:#ffe8ea}.tool-icon{width:34px;height:34px;margin-bottom:10px;font-size:19px;transition:transform .2s ease,background .2s ease}.tool-card h3{font-size:15px}.tool-card p{font-size:11px;line-height:1.35}.tool-arrow{padding-top:10px;font-size:11px}.tool-workflow{position:absolute;top:10px;right:12px;color:#9aa5af;font-size:8px;font-weight:700;letter-spacing:.8px;text-transform:uppercase}.tool-card:hover .tool-workflow{color:#db0011}.home-header{padding-top:24px;padding-bottom:18px}.home-logo{width:58px;height:58px}.home-header h1{font-size:34px}.home-footer{padding-top:14px;padding-bottom:14px}@media(max-width:1100px){.tool-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:820px){.tool-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:620px){.tool-grid{grid-template-columns:1fr}}';document.head.appendChild(homeStyle);
