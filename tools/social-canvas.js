(function(){
  ...
  function render(){var query=document.getElementById('size-search').value.toLowerCase(),grid=document.getElementById('size-grid');var filtered=sizes.filter(function(item){return(activePlatform==='all'||item.platform===activePlatform)&&(!query||(item.platform+' '+item.format+' '+item.usage).toLowerCase().includes(query));});grid.innerHTML=filtered.length?filtered.map(function(item){return'<article class="size-card"><div class="size-card-top"><span class="platform-label">'+item.platform+'</span><span class="usage-label">'+item.usage+'</span></div><h2>'+item.format+'</h2><div class="size-number">'+item.width+' × '+item.height+' <small>px</small></div><div class="size-meta"><span>Ratio '+ratio(item.width,item.height)+'</span>'+(item.maxHeight?'<span class="max-height">Max height '+item.maxHeight+' px</span>':'')+'</div><div class="size-actions"><button class="copy-size" data-value="'+item.width+' × '+item.height+' px">Copy size</button><a href="image-studio.html?preset='+item.width+'x'+item.height+'">Use in Image Studio</a></div></article>';}).join(''):'<div class="no-results">No matching sizes.</div>';grid.querySelectorAll('.copy-size').forEach(function(button){button.addEventListener('click',function(){copy(button.dataset.value);button.textContent='Copied';setTimeout(function(){button.textContent='Copy size';},1200);});});}
