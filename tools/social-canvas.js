(function(){
  ...
  function templateMargins(item){var story=item.usage.toLowerCase().includes('story')||item.aliases&&item.aliases.includes('vertical');return{left:Math.round(item.width*.08),right:Math.round(item.width*.08),top:story?Math.round(item.height*.12):Math.round(item.height*.06),bottom:story?Math.round(item.height*.12):Math.round(item.height*.06),danger:story?Math.round(item.height*.1):0};}
  function downloadTemplate(item){var canvas=document.createElement('canvas'),ctx=canvas.getContext('2d'),m=templateMargins(item);canvas.width=item.width;canvas.height=item.height;ctx.fillStyle='#ffffff';ctx.fillRect(0,0,item.width,item.height);ctx.strokeStyle='#db0011';ctx.lineWidth=Math.max(2,Math.round(item.width/540));ctx.setLineDash([Math.max(8,Math.round(item.width/60)),Math.max(5,Math.round(item.width/100))]);ctx.strokeRect(m.left,m.top,item.width-m.left-m.right,item.height-m.top-m.bottom);ctx.setLineDash([]);ctx.fillStyle='#db0011';ctx.font='700 '+Math.max(18,Math.round(item.width/54))+'px Arial';ctx.textAlign='center';ctx.fillText('SAFE AREA',item.width/2,m.top+Math.max(28,Math.round(item.height/45)));if(m.danger){ctx.fillStyle='rgba(219,0,17,.11)';ctx.fillRect(0,0,item.width,m.danger);ctx.fillRect(0,item.height-m.danger,item.width,m.danger);ctx.fillStyle='#db0011';ctx.font='700 '+Math.max(16,Math.round(item.width/68))+'px Arial';ctx.fillText('DANGER ZONE · KEEP IMPORTANT CONTENT OUT',item.width/2,m.danger/2+6);ctx.fillText('DANGER ZONE · KEEP IMPORTANT CONTENT OUT',item.width/2,item.height-m.danger/2+6);}ctx.fillStyle='#71808c';ctx.font='500 '+Math.max(14,Math.round(item.width/90))+'px Arial';ctx.textAlign='left';ctx.fillText(item.width+' × '+item.height+' px · '+item.format,m.left,m.top+Math.max(50,Math.round(item.height/28)));var link=document.createElement('a');link.download='template-'+item.platform.toLowerCase().replace(/[^a-z0-9]+/g,'-')+'-'+item.width+'x'+item.height+'.png';link.href=canvas.toDataURL('image/png');link.click();}
  function render(){var query=document.getElementById('size-search').value.toLowerCase(),grid=document.getElementById('size-grid');var filtered=sizes.filter(function(item){return(activePlatform==='all'||item.platform===activePlatform)&&(!query||(item.platform+' '+item.format+' '+item.usage+' '+(item.aliases||[]).join(' ')).toLowerCase().includes(query));});grid.innerHTML=filtered.length?filtered.map(function(item){var margins=templateMargins(item);return'<article class="size-card"><div class="size-card-top"><span class="platform-label">'+item.platform+'</span><span class="usage-label">'+item.usage+'</span></div><h2>'+item.format+'</h2><div class="size-number" title="Click a number to copy"><button class="size-part" data-value="'+item.width+' px">'+item.width+'</button><span>×</span><button class="size-part" data-value="'+item.height+' px">'+item.height+'</button><small>px</small></div><div class="size-meta"><span>Ratio '+ratio(item.width,item.height)+'</span><span>Safe L/R '+margins.left+' px</span>'+(margins.danger?'<span class="max-height">Danger top/bottom '+margins.danger+' px</span>':'')+(item.maxHeight?'<span class="max-height">Max height '+item.maxHeight+' px</span>':'')+'</div><div class="size-actions"><button class="copy-size" data-value="'+item.width+' × '+item.height+' px">Copy size</button><button class="download-template" data-index="'+sizes.indexOf(item)+'">Download template</button></div></article>';}).join(''):'<div class="no-results">No matching sizes.</div>';grid.querySelectorAll('.copy-size,.size-part').forEach(function(button){button.addEventListener('click',function(){copy(button.dataset.value);var original=button.textContent;button.textContent='Copied';setTimeout(function(){button.textContent=original;},1200);});});grid.querySelectorAll('.download-template').forEach(function(button){button.addEventListener('click',function(){downloadTemplate(sizes[Number(button.dataset.index)]);});});}
